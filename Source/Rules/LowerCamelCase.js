...
function CheckVariable(context, node) {
  ...
  if (!new RegExp(common.lower_camel_case_regex, `g`).test(variable_name)) {
    context.report({
      node: node,
      ...
      fix: function (original_fixer) {
        ...
        if (new RegExp(common.lower_camel_case_regex).test(variable_name))
          new_variable_name = variable_name.replace(/^[a-z]/, (match) =>
            match.toUpperCase()
          );
        else if (new RegExp(common.snake_case_regex).test(variable_name))
          new_variable_name = variable_name.replace(/_[a-z]/g, (match) =>
            match[1].toUpperCase()
          );
        else return null;
    ...
    return {
      ...
      VariableDeclaration: function (node) {
        ...
        node.declarations.forEach((declaration) => {
          ...
          if (variable_whitelist.indexOf(variable_name) > -1) return;
          ...
          if (
            ...
            if (declaration.init.arguments[0].type === `TemplateLiteral`)
              ...
              module_name =
                declaration.init.arguments[0].quasis[0].value.cooked;
      ...
      Identifier: function (node) {
        const ALLOWED_PARENTS = [`FunctionDeclaration`, `MethodDefinition`];
        if (ALLOWED_PARENTS.indexOf(node.parent.type) === -1) return;
        ...
        if (
          node.parent.type === ALLOWED_PARENTS[0] &&
          node.name !== node.parent.id.name
        )
          return;
        // Constructors have to be named the way they are.
        if (
          node.parent.type === ALLOWED_PARENTS[2] &&
      ...
      CallExpression: function (node) {
        if (
          node.callee.object &&
          variable_whitelist.indexOf(node.callee.object.name) > -1
        )
