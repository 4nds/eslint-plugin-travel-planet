...
const lower_camel_case_regex = FormatRegexCaptureGroup(
  `[a-z](${CAMEL_CASE_COMMON_REGEX})*`
);
// Underscores are allowed, for CONSTANT_VARIABLE_NAME.
const upper_camel_case_regex = FormatRegexCaptureGroup(
  `[A-Z](${CAMEL_CASE_COMMON_REGEX}|_)*`
);

const snake_case_regex = FormatRegexCaptureGroup(`[a-z]([a-z]|[0-9]|_)*`);

function MakeVariableNameFixers(
  original_fixer,
  source_code_text,
  variable_name,
  new_variable_name
  ...
  var fixers = [];
  while (
    (match_start = source_code_text.indexOf(variable_name, match_end)) > -1
  ) {
    match_end = match_start + variable_name.length;
    fixers.push(
      original_fixer.replaceTextRange(
